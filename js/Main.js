...
$.get("https://personal-chat-3777.twil.io/get_token?user=" + user_id, function(token, status) {
    accessToken = token;
    loadertest();
    Twilio.Chat.Client.create(accessToken)
        .then(chatClient => {
            client = chatClient;
            loadertest();
            updateJoinedChannel();
...
function insertGroupChat(who, text, time, date, author) {
    ...
    var avatar = member ? member.avatar : default_avather;
    ...
    if (who == "other") {
        control = seperate(date, author) + '<li style="width:100%;padding-left: 15px;">' +
            ...
            '<div class="authoryou" style="color:' + color + '"><b>' + author + '</b></div>' +
            '<p>' + text + '</p>' +
            '<p>' + chattime + '</p>' +
            '<br></div>' +
            '</div>' +
...
function seperate(date, author) {
    ...
    if (tempdatediff != diffDays) {
        if (diffDays == 1) {
            seperator = '<br><li><div class="datediveder">----Yesterday----</div></li><br>';
        } else if (diffDays < 1) {
            seperator = '<br><li><div class="datediveder">----Today----</div></li><br>';
        } else {
            seperator = '<br><li><div class="datediveder">----' + day + '/' + mnth + '/' + year + '----</div></li><br>';
        }
    }
    tempauthor = author;
...
function search() {
    ...
    client.getUser(name).then(UserDetails => {
            console.log(UserDetails);
            $('.searchresultbox').append('<div class="searchresult" onclick="checkchannel(\'' + UserDetails.identity + '\')">Name : ' + UserDetails.identity + '</div>');
        })
...
function checkchannel(user) {
    ...
    client.getChannelByUniqueName(channelName1)
        .then(channel => alert("Chat already exist"))
        .catch(e => {
            client.getChannelByUniqueName(channelName2)
                .then(channel => alert("Chat already exist"))
